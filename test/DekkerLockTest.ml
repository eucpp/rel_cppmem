...
let test_SC =
  ...
  let initstate = Interpreter.(ProgramState.make prog_DekkerLock @@ State.init regs mem) in
  let asserto t =
    fresh (s m)
  ...
  let test () =
    ...
    if Stream.is_empty stream then
      ...
      Format.printf "Dekker Lock test (%s) fails!@;" name;
...
let tests = Test.make_testsuite ~name:"DekkerLock" ~tests:
  [
    test_SC;
    test_RA;
  ]
