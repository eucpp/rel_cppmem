...
let make_test_desc ?n ?locs ?mem ?len ~regs ~name ~stat ~prop prog =
  { name; prog; regs; locs; mem; stat; prop; n; len }
...
module OperationalTest(Memory : Operational.MemoryModel) =
  struct
    ...
    let make_testcase { name; prog; prop; regs; locs; mem; stat; n; len } =
